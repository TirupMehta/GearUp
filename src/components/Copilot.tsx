...
User Question: ${userText}
  ...
  return (
    <aside className="w-[360px] shrink-0 bg-panel/60 backdrop-blur-xl border-l border-border rounded-3xl flex flex-col h-full right-0 top-0 hidden xl:flex absolute xl:relative z-20">
      <div className="p-4 border-b border-border flex items-center gap-2 bg-panel/30 backdrop-blur-sm">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col hide-scrollbar">
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center ${
              m.role === 'user' ? 'bg-primary' : 'bg-text-muted/20'
            }`}>
            ...
            <div className={`p-3 rounded-2xl max-w-[85%] text-sm ${
              m.role === 'user' 
                ...
                : 'bg-background border border-border text-text rounded-tl-none prose prose-sm'
            }`}>
              {m.role === 'user' ? (
                m.text
              ) : (
        ...
        {isTyping && (
           <div className="flex gap-3">
              <div className="w-8 h-8 rounded-full shrink-0 flex items-center justify-center bg-text-muted/20">
                <BrainCircuit className="w-4 h-4 text-accent" />
              </div>
              <div className="p-3 py-4 rounded-2xl bg-background rounded-tl-none border border-border flex gap-1 items-center">
      ...
      <div className="p-4 border-t border-border bg-panel/60">
        
        <button 
          onClick={generateBattleplan}
          disabled={isTyping}
          className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-primary to-accent hover:opacity-90 text-white font-medium py-2 rounded-2xl mb-4 shadow-lg shadow-primary/20 transition-all text-sm disabled:opacity-50"
        >
        ...
        <div className="relative flex items-center gap-2">
          ...
          <button 
            ...
            className="p-2 shrink-0 text-text-muted hover:text-accent hover:bg-background rounded-2xl border border-border transition"
          ...
          <input 
            ...
            className="w-full bg-background border border-border rounded-full py-2 pl-4 pr-10 text-sm focus:outline-none focus:border-primary text-text"
          />
          <button 
            onClick={handleSend}
            disabled={!input.trim() || isTyping}
            className="absolute right-1 top-1 p-1.5 bg-primary text-white rounded-full hover:bg-primaryHover disabled:bg-text-muted transition"
          >
