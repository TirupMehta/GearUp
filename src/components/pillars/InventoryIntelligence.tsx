    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#000000]/40 border border-white/5 rounded-2xl overflow-hidden shadow-[0_0_20px_rgba(255,255,255,0.05)]">
          <div className="bg-white/5 backdrop-blur-lg px-6 py-4 border-b border-white/5">
          ...
          <div className="p-0 overflow-auto max-h-80">
            <table className="w-full text-sm text-left">
              <thead className="bg-[#000000]/20 text-zinc-400 sticky top-0">
                <tr>
                  ...
                  <tr key={idx} className="hover:bg-[#000000]/30 backdrop-blur-sm">
                    ...
                    <td className="px-6 py-3 text-right">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
                        (stockMap[item.ProductName] || 0) < 20 ? 'bg-danger/20 backdrop-blur-sm shadow-xl shadow-danger/20 text-danger border border-danger/30' : 'bg-white/5 text-zinc-300'
                      }`}>
              ...
              <tbody className="divide-y divide-slate-700/50">
                {deadWeight.slice(0, 10).map((item, idx) => (
                  <tr key={idx} className="hover:bg-[#000000]/30 backdrop-blur-sm">
                    <td className="px-6 py-3 font-medium text-slate-200">{item.ProductName}</td>
                    <td className="px-6 py-3 text-right">
                      <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-white/5 text-zinc-300">
                        {(stockMap[item.ProductName] || 0)} units
                      </span>
                    </td>
