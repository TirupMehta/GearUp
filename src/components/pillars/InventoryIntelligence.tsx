...
export default function InventoryIntelligence() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Alert Cards */}
      {trend < 0 && (
        <div className="bg-warning/10 border border-warning/30 rounded-xl p-4 flex gap-4 items-start">
        ...
        <div className="bg-slate-900 border border-slate-700/50 rounded-xl overflow-hidden shadow-lg">
          ...
          <div className="p-0 overflow-auto max-h-80">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-800/20 text-slate-400 sticky top-0">
                <tr>
                  ...
                  <tr key={idx} className="hover:bg-slate-800/30">
                    ...
                    <td className="px-6 py-3 text-right">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
                        (stockMap[item.ProductName] || 0) < 20 ? 'bg-danger/20 text-danger border border-danger/30' : 'bg-slate-800 text-slate-300'
                      }`}>
